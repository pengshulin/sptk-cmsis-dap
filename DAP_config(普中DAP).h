...
LED_CNT：PB12 Green
LED_RUN：PB11 Blue
TRST:  PB9   output (open drain)  pull-up
SWDIO: PA1   i/o                  pull-up
SWCLK: PA5   output               pull-down
TDO:   PA6   input                pull-up
TDI:   PA7   output               pull-up
SRST:  PB0   output (open drain)
*********************************/
...
{
    // PA1, PA5, PA7 output high
    GPIOA->CRL  &= ~0xF0F000F0;
    GPIOA->CRL  |=  0x70300070;
    GPIOA->BSRR  =  0x00A2;
    // PB0 output high
    GPIOB->CRL  &= ~0x0000000F;
    GPIOB->CRL  |=  0x00000007;
    GPIOB->BSRR  =  0x0001;
    // PB9 output high
...
{
    // PA1, PA5 output high
    GPIOA->CRL  &= ~0x00F000F0;
    GPIOA->CRL  |=  0x00300070;
    GPIOA->BSRR  =  0x0022;
    // PB0 output high
    GPIOB->CRL  &= ~0x0000000F;
    GPIOB->CRL  |=  0x00000007;
...
{
    // PA1, PA5, PA6, PA7 as floating input
    GPIOA->CRL &= ~0xFFF000F0;
    GPIOA->CRL |=  0x44400040;
    // PB0 as input
    GPIOB->CRL &= ~0x0000000F;
...
{
    GPIOA->CRL = (GPIOA->CRL & ~(0x000000F0)) | (0x00000070);  // PA1 50M/OpenDrain
}
...
{
    GPIOA->CRL = (GPIOA->CRL & ~(0x000000F0)) | (0x00000040);  // PA1 Float IN
}
...
{
    RCC->APB2ENR |= (1UL<<3) | (1UL<<2);  // PA PB

    GPIOB->CRH   &= ~0x000FF000;  // PB11 PB12
    GPIOB->CRH   |=  0x00011000;
    GPIOB->BRR = 0x1800;

    PORT_OFF();  // Disable all
}
